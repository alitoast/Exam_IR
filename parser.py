...
class Parser:
    ...
    def parse_sitemap(self, sitemap_list):
        '''
        Input:
        ...
        for sitemap_url in sitemap_list:
            if sitemap_url:
                ...
                for entry in entries:
                    url = self.normalize_url(entry['url']) #normalizzo l'url
                    if self.check_spider_traps(url): # provo a limitare le spider-trap
                        entry['url'] = url
    ...
    async def extract_links(self, html, base_url=None):
